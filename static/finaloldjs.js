// like button begin
const likeQueryAll = document.querySelectorAll('.like-button');
likeQueryAll.forEach(query => {
    query.addEventListener('click', (e) => {
        // e is nothing but event object parameter. .currentTarget gets the element that invoked the event ie. button
        // classList gives its list of classes.
        // toggle shifts that class to liked. 
        e.currentTarget.classList.toggle('liked');
        var len=e.currentTarget.classList.length;
        var songnum = e.currentTarget.getAttribute("like-button-track")
        const findTitleTrack = document.querySelectorAll('a');
        console.log(songnum)
        console.log(len)
        // console.log(findTitleTrack[1].getAttribute("data-play-track"))
        // console.log(findTitleTrack[10].innerHTML)
        var i;
        var songname;
        for (i =0; i < findTitleTrack.length; i++)
        {
          var j = findTitleTrack[i].getAttribute("data-play-track");
          if(songnum==j)
          {
            var songname=findTitleTrack[i].innerHTML
            console.log(findTitleTrack[i].innerHTML)
          }
        }
        // console.log(e.currentTarget.getAttribute("like-button-track"));
        // console.log(e.currentTarget)
        //var datadict={
        //  "len":len,
        //  "songnum":likeIndexQuery,
        //  "songname":songname,
        //};
        $.ajax({
          ...
          data: JSON.stringify({"len":len, "songnum":songnum, "songname":songname })
        })
    });
})
...
// player begin
  ...
  var _bufferProgress = function(audio) {
    if(audio.onprogress)
    {
      var bufferedTime = 100*(audio.buffered.end(0)) / audio.duration;
...
// new function
   ...
   * @param playListRows The playlist object.
   **/
  var _setTrackTitleforStopTime = function(currentTrack, playListRows) {
    // var trackTitleBox = document.querySelector(".player .info-box .track-info-box .track-title-text");
    var trackTitle1 = playListRows[currentTrack - 1].children[2].outerText;

    // trackTitleBox.innerHTML = null;

    // trackTitleBox.innerHTML = trackTitle;

    document.title = trackTitle1;
...
// end of new function 
  ...
  var _trackHasEnded = function() {
    // code for displaying stop time
    var currentTime1 = audio.currentTime;
    var trackCurrentTime1 = _trackTime(currentTime1);
    console.log(trackCurrentTime1);
    console.log(_currentTrack);
    var trackTitle1 = _setTrackTitleforStopTime(_currentTrack, _elements.playListRows);
    // end of code for displaying stop time
    $.ajax({
      url:'/trackTiming',
      type:"POST",
      contentType: "application/json",
      data: JSON.stringify({"trackCurrentTime1":trackCurrentTime1, "_currentTrack":_currentTrack, "trackTitle1":trackTitle1})
    })
...
// script for sliders ie. personalised start
...
var z = document.getElementById("myRange1");
...
var x = document.getElementById("myRange3");
...
var w = document.getElementById("myRange4");
var d = document.getElementById("demo4");
d.innerHTML = w.value;
w.oninput = function() {
    d.innerHTML = this.value;
}

var u = document.getElementById("myRange6");
...
// script for sliders end

// script for bottom nav bar start
function myFunction() {
